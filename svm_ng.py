...
f = open('data/ng3tr5_vocab_5.txt', 'r')
vocab = f.read().splitlines()

f = open('data/ng3tr5_classes_5.txt', 'r')
train_labels = f.read().splitlines()
train_labels = [int(label) for label in train_labels]

train_set = []
with open('data/ng3tr5_bag_of_words_5.csv') as csvfile:
...
f = open('data/ng3te5_classes_0.txt', 'r')
test_labels = f.read().splitlines()
test_labels = [int(label) for label in test_labels]

test_set = []
with open('data/ng3te5_bag_of_words_0.csv') as csvfile:
    reader = csv.reader(csvfile)
    for row in reader:
